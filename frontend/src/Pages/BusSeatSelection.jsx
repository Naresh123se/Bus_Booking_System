...
const handleFormSubmit = async () => {
  try {
    ...
    const userId = user?.name; // Assuming userId is stored in 'name'
    if (!userId) {
      throw new Error('User not authenticated or userId not found');
    }

    console.log(userId); // Logging userId to verify it's retrieved correctly
    const response = await Selseat({ userId, selectedSeats }).unwrap(); // Calling seat mutation with userId and selectedSeats
    console.log(response); // Logging the response to check if data is sent correctly
